...
(function() {
  ...
  AstNode = class AstNode {
    ...
    next(Node, name, token) {
      this.syntax_check(Node, name, token);
      this.semantic_check(Node, name, token);
    ...
    syntax_check(Node, name, token) {
      ...
      if (ref = Node.prototype.type, indexOf.call(this.child_type, ref) < 0) {
        throw new SemanticError(`invalid Node => [name: ${name}, token: ${JSON.stringify(token)}, type: ${Node.prototype.type}],\nthe accepted child type of the parent [name: ${this.name}, token: ${JSON.stringify(this.token)}, type: ${this.type}] must be included in ${this.child_type}`);
      }
    }
    ...
    class Field extends AstNode {
      ...
      semantic_check(Node) {
        ...
        if (this.parent.related_field_name != null) {
          throw new SemanticError(`for node [name: ${this.name}, token: ${JSON.stringify(this.token)}, type: ${this.type}],\nstart spreading a new field <${this.name}>, but a previous field [${this.parent.related_field_name}] has been found, can not specify other \`Field\` type inside one \`Field\``);
        } else if (this.children.length > 0) {
          throw new SemanticError(`the Field node [name: ${this.name}, token: ${JSON.stringify(this.token)}, type: ${this.type}] can not have multiple child`);
        }
      }
    ...
    class UnaryLogicalOp extends AstNode {
      ...
      semantic_check(Node) {
        ...
        if (this.children.length > 0) {
          throw new SemanticError(`unary logical operator node [name: ${this.name}, token: ${JSON.stringify(this.token)}, type: ${this.type}] can't have more than one child`);
        }
      }
  ...
  UnaryRelationOp = (function() {
    class UnaryRelationOp extends AstNode {
      ...
      semantic_check(Node) {
        super.semantic_check(Node);
        if (this.parent.related_field_name != null) {
          throw new SemanticError(`unary relation operator node [name: ${this.name}, token: ${JSON.stringify(this.token)}, type: ${this.type}] can't be used under a Field node, but previous related field [${this.parent.related_field_name}] has been found`);
        }
        if (this.children.length > 0) {
          throw new SemanticError(`unary relation operator node [name: ${this.name}, token: ${JSON.stringify(this.token)}, type: ${this.type}] can't have more than one child`);
        }
      }
